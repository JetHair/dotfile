...
in
pkgs.writeShellScriptBin "nixi" ''
  #!${pkgs.bash}/bin/bash
  set -euo pipefail

  # --- Program info ---
  #
  # nixi - NixOS System Management CLI
  # ==================================
  #
  # Architecture:
  ...
  verify_hostname() {
    ...
    if [ -f "$FLAKE_NIX_PATH" ]; then
      ...
      if [ "$current_hostname" != "$flake_hostname" ]; then
        ...
        echo "" >&2
        echo "Hint: Run 'nixi update-host' to automatically update flake.nix" >&2
  ...
  print_help() {
    ...
    echo ""
    echo "Usage: nixi [command] [options]"
    echo ""
    echo "Commands:"
    ...
    echo "                    (Opt: nixi update-host [hostname] [profile])"
    echo ""
  ...
  case "$1" in
    cleanup)
      ...
      LOG_DIR="$HOME/cleanup-logs"
      mkdir -p "$LOG_DIR"
      LOG_FILE="$LOG_DIR/cleanup-$(${pkgs.coreutils}/bin/date +%Y-%m-%d_%H-%M-%S).log"
    ...
    update-host)
      ...
      if [ "$#" -eq 3 ]; then # nixi update-host <hostname> <profile>
        target_hostname="$2"
        target_profile="$3"
      elif [ "$#" -eq 1 ]; then # nixi update-host (auto-detect)
        ...
        if [ -z "$target_profile" ]; then
          ...
          echo "Usage: nixi update-host [hostname] [profile]" >&2
          exit 1
        fi
